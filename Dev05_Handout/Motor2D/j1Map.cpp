	...
	for (tileset = data.tilesets.start; tileset; tileset = tileset->next)
	{
		for (p2List_item<MapLayer*>* layer = data.layers.start; layer; layer = layer->next)
		{
			for (uint y = 0; y < layer->data->height; ++y)
			{
				...
				{
					iPoint pos_in_world = MapToWorld_ISO(iPoint(x, y));
					SDL_Rect section = tileset->data->GetTileRect(layer->data->GetID(x, y));
...
iPoint j1Map::MapToWorld_ISO(const iPoint& p) const
{
	iPoint ret;
...
iPoint j1Map::WorldToMap_ISO(const iPoint& p) const
{
	iPoint ret;

	ret.x = ((p.x / data.tile_width) + (p.y / data.tile_height )) * 0.5f;
	ret.y = ((p.y / data.tile_height) - (p.x / data.tile_width )) * 0.5f;
