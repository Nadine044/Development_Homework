...
void j1App::FinishUpdate()
{
	...
	sprintf_s(title, 256, "Av.FPS: %.2f Last Frame Ms: %02u Last sec frames: %i Last dt: %02u Time since startup: %.3f Frame Count: %lu ",
	...
	if (framerate_cap > 0 && last_frame_ms < framerate_cap)
	{
		...
		j1PerfTimer amount_of_time;

		SDL_Delay(framerate_cap - last_frame_ms);
		LOG("We waited for %d milliseconds and got back in %f", framerate_cap - last_frame_ms, amount_of_time.ReadMs());
	}
