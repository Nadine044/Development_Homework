...
void j1Map::Draw()
{
	...
	p2List_item<MapLayer*>* layer1 = nullptr;
	...
	for (tileset = data.tilesets.start; tileset; tileset = tileset->next)
	{
		for (p2List_item<MapLayer*>* layer = data.layers.start; layer; layer = layer->next)
		{
			for (uint y = 0; y < layer->data->height; ++y)
			{
				...
				{
					iPoint pos_in_world = MapToWorld(x, y);
					SDL_Rect section = tileset->data->GetTileRect(layer->data->Get(x, y));
...
TileSet* j1Map::GetTilesetFromTileId(int id) const
{
	...
	/*Notice that we are using more than one tileset 
	Our code reads them but we cannot draw them! 
	The attribute ﬁrstgid sets the ﬁrst tile id that this tileset contains 
	Knowing this, ﬁll the method that returns the proper tileset attached to any tile id*/
